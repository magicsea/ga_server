...
func (s *SessionService) OnUserLogin(context service.Context) {
	...
	context.Tell(context.Sender(), &gameproto.UserLoginResult{Uid: uint32(msg.Uid), GateAddr: gateAddr, Key: key, Result: int32(msgs.OK)})
}
...
func (s *SessionService) OnUserCheckLogin(context service.Context) {
	...
	log.Println("SessionService.OnUserCheckLogin pre:", msg.Uid)
	/*
		//安装
...
func (s *SessionService) OnUserCheckLoginGsBack(context service.Context) {
	...
	log.Println("SessionService.OnUserCheckLogin ok:", id)
}
